...
static int read_mac(char *filename,unsigned char *buf, size_t len)
{
    struct stat statbuf;
    FILE *genmac;

    if (!stat(filename, &statbuf)) {
        genmac = fopen(filename, "r");
...
int wcnss_qmi_get_wlan_address(unsigned char *pBdAddr)
{
    ...
    rc = read_mac(WCNSS_GENMAC_FILE, buf, 6);
    if (rc == SUCCESS) {
    ...
    rc = read_mac(WCNSS_MAC_FILE, buf, 6);
    if (rc != SUCCESS ||
