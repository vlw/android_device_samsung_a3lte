/******************************************************************************
 *
 *  $Id: FileIO.c 1032 2013-06-12 09:23:43Z yamada.rj $
 *
 * -- Copyright Notice --
 *
 ...
 */
int16 LoadParameters(const char* path, AKSCPRMS * prms)
{
	int16	i, ret;
...
*/
int16 LoadPDC(const char* path, AKSCPRMS* prms)
{
	int16   ret, i;
	FILE*   fp;
	char    keyName[KEYNAME_SIZE];
	...
	AKMDEBUG(AKMDBG_LEVEL1, "Parameter loaded: %s\n", header);
 ...
 */
int16 SaveParameters(const char* path, AKSCPRMS * prms)
{
	int16	ret = 1;
	int16	i;
#ifndef AKMD_DISABLE_DOEPLUS
